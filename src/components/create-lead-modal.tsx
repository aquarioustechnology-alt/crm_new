...
export function CreateLeadModal({ isOpen, onClose, onLeadCreated }: CreateLeadModalProps) {
  ...
  async function createLead() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMsg(err.error || `Failed to create lead (HTTP ${res.status})`);
      }
    } catch (e: any) {
      setMsg(e?.message || "Network error while creating lead.");
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-[1000]">
      <div className="absolute inset-0 z-[1000] bg-black/80" />
      <div className="fixed z-[1100] left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[960px] max-w-full max-h-[90vh] overflow-y-scroll bg-slate-800 border border-slate-600 rounded-lg p-6 shadow-xl transform-gpu will-change-transform overscroll-contain" style={{ scrollbarGutter: 'stable both-edges' }}>
        <div>
        ...
        <form className="mt-6 min-h-[360px]" onKeyDown={(e) => {
          if (e.key === 'Enter') {
            e.preventDefault();
          ...
          {currentStep === 2 && (
            <div className="grid grid-cols-2 gap-6" style={{ contain: 'layout paint' }}>
              <div className="space-y-4">
                <div>
              ...
              <div className="col-span-1">
                ...
                <Textarea 
                  ...
                  className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 rounded-lg h-full"
                />
              </div>
            </div>
          )}
